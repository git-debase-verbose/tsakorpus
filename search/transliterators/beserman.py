...
rxNeutral2Capital = re.compile('([бвгжӝкмпрфхцчӵʼаоэӥуўяёеиюөБВГЖӜКМПРФХЦЧӴАОЭӤУЎЯЁЕИЮӨ]|\\b)(Ӥ)')
rxNeutral2 = re.compile('([бвгжӝкмпрфхцчӵʼаоэӥуўяёеиюөБВГЖӜКМПРФХЦЧӴАОЭӤУЎЯЁЕИЮӨ]|\\b)(ӥ)')
...
rxVJV = re.compile('(?<=[аеёиӥоӧөуыэюяйь])й([аэоу])', flags=re.I)
...
def beserman_translit_cyrillic(text):
    ...
    res = rxNeutral2Capital.sub('\\1И', res)
